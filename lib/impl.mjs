...
export function appendTwinRecord(agent, st, res, { kind, log, turn, step } = {}) {
  ...
  const content = [
    // 上限 `twinRecordChars`：思考正文会随这条记录留在会话里、之后每一轮请求都要重发一遍
    // （实测溢出事故：messages 793541 + completion 256000 > 1048576 上限被上游拒收）。
    // 全文仍旧进 jsonl 备份，会话里只留一段，避免我们自己把会话往窗口上限上推。
    { type: 'reasoning', text: `〔监察思考〕${clipText(String(res.thinking || '').trim(), Number(loadConfig().twinRecordChars ?? 1200)) || '（本次监察没有留下思考正文）'}` },
    { type: 'text', text },
  ]
...
/** 尾巴能不能写（在 pairingOk 之外再加一层"最后一条不许是没配齐的调用"）。 */
  ...
  for (const item of items) {
    try {
      if (item.kind === 'record') {
        ...
        // 落盘可见性：一条 info 日志，让"监察思考真的进了会话"这件事可被抽查
        // （落地时的 seq 拿不到就报估算，只作观测用）
        log?.info?.(`[${PLUGIN_NAME}] 监察记录已落盘：turn ${item.turn} step ${item.step}（${item.reviewKind}）`)
