...
export const DEFAULTS = {
  ...
  // ── 教训注入（2026-09-19 自流变·记忆移交）────────────────────────────
  // 数据由流变·记忆的 lessons 工具蒸馏/维护（~/.dsh/liubian/lessons*.json），
  // 本插件只负责把清单注入执行侧上下文（会话开始一次 + 每 N 轮重注）。
  twinLessonsInject: true,
...
export function makeState() {
  return {
    ...
    // 教训注入（自流变·记忆移交）：状态跨回合保留，不随 turn 重置
    lessonsDelivered: false,     // 会话开始那次是否已经送出（含"无教训可发"的情况）
    lessonsCounter: 0,           // 距上次重注过了几个人类轮次
    lessonsLastHumanCount: 0,    // 上次计轮时的人类消息条数（同一步重试条数不变，天然防重）
  }
}
...
/* ──────────────────────────────────────────────────────────────────────────
 * 8.5 教训注入（2026-09-19 自流变·记忆移交）
 *
 * 分工：教训的**生产**（从记忆库蒸馏、list/add/remove）留在流变·记忆的
 *   `_dsh_external_dsh_liubian_lessons` 工具；本插件只负责把清单**注入**执行侧
 *   上下文（会话开始一次 + 每 N 个人类轮次重注）。
 * 交接面：~/.dsh/liubian/lessons*.json（{updatedAt, lessons:[…]}），本插件只读。
 * 块格式与流变·记忆时代逐字一致（含块头标签与两条入口提示），模型侧无感。
 * ────────────────────────────────────────────────────────────────────────── */
...
/** 会话的工作区名：session.header.cwd 的最后一段（与流变·记忆同源）；拿不到则只发全局块。 */
...
/** 注入块：条数与总长有上限；空清单返回 ''（不注入）。格式与流变·记忆时代逐字一致。
 *  工作区块在拿不到工作区名（ws 为空）时返回 '' —— 否则会把全局清单顶着空标签复读一遍。 */
export function buildLessonsBlock(cfg, scope = 'global', ws = '') {
  if (scope === 'workspace' && !ws) return ''
  const lessons = loadLessons(cfg, scope, ws)
...
/** 人类消息判定（照快查表 3.3.2：role=user 且 source.kind 为空或 'user'；注入块/技能目录/工具结果都不算）。 */
function isHumanMessage(m) {
  return !!m && m.role === 'user' && (!m.source || !m.source.kind || m.source.kind === 'user')
}

/** 当前该注入的教训文本（全局 + 工作区两块拼好）；没有可发的返回 ''。 */
function lessonsTextFor(cfg, agent) {
...
/**
 * 教训注入的两条接线（会话开始一次 + 每 N 个人类轮次重注）。
 * 纪律同全局：监听器里除 await next() 外整段 try/catch，绝不能拖垮执行侧的回合。
 */
export function mountLessons(ctx, cfg, log) {
  ctx.on('agent/session-start', ({ agent }) => {
    try {
      if (!cfg.twinLessonsInject || !agent) return
      const st = stateFor(agent.session?.id)
      const text = lessonsTextFor(cfg, agent)
      if (!text) { st.lessonsDelivered = true; return } // 没有可发的也置位，免得每步重读文件
      if (agent.status && agent.status !== 'idle') return // 忙：**不置位**，留给 pre-step 兜底
      try {
        agent.inject(userMessage(text, 'recall'))
        st.lessonsDelivered = true
        st.lessonsCounter = 0
        st.lessonsLastHumanCount = 0
      } catch (err) {
        // 注入失败不置位：pre-step 会兜底补一次
        log?.debug?.(`[${PLUGIN_NAME}] 教训会话开始注入失败（留给 pre-step 兜底）：${err?.message || err}`)
      }
    } catch (err) {
      log?.warn?.(`[${PLUGIN_NAME}] 教训会话开始注入异常（忽略）：${err?.message || err}`)
    }
  })
  ...
  ctx.on('agent/pre-step', async ({ agent, messages, signal }, next) => {
    ...
    try {
      ...
      if (!cfg.twinLessonsInject || !agent?.session?.id) return decision
      const st = stateFor(agent.session.id)
      const humanCount = (decision.messages || []).filter(isHumanMessage).length
      if (!humanCount) return decision
      // 会话开始那次没送成（当时忙 / inject 抛错）→ 这里兜底补一次
      if (!st.lessonsDelivered) {
        st.lessonsDelivered = true
        st.lessonsCounter = 0
        st.lessonsLastHumanCount = humanCount
        const text = lessonsTextFor(cfg, agent)
        if (text) return { kind: 'enter', messages: [...decision.messages, userMessage(text, 'recall')] }
        return decision
      }
      // 周期重注：每 N 个人类轮次一次；同一步重试条数不变，天然防重
      const every = Math.max(0, Number(cfg.twinLessonsEveryTurns) || 0)
      if (!every || humanCount === st.lessonsLastHumanCount) return decision
      st.lessonsLastHumanCount = humanCount
      st.lessonsCounter += 1
      if (st.lessonsCounter >= every) {
        st.lessonsCounter = 0
        const text = lessonsTextFor(cfg, agent)
        if (text) return { kind: 'enter', messages: [...decision.messages, userMessage(text, 'recall')] }
      }
      ...
      log?.warn?.(`[${PLUGIN_NAME}] 教训注入异常（忽略，不影响本轮）：${err?.message || err}`)
      return decision
    }
  })
}
...
export function apply(ctx, input = {}) {
  ...
  // 教训注入（自流变·记忆移交）：会话开始一次 + 每 N 个人类轮次重注
  mountLessons(ctx, cfg, log)
