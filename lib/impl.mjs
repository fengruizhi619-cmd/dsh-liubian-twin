...
export const DEFAULTS = {
  ...
  twinBackgroundChars: 1500, // 背景/记录的字符上限；0 = 连背景都不给
  twinTranscriptMessages: 6, // flatten 模式下拍平多少条最近消息
...
/** 一条消息里的可读文本（text + reasoning），供"拍平成一段普通文本"用。 */
function readableText(msg) {
  if (!msg) return ''
  ...
  return [thinking && `（它的思考）${thinking}`, text, ...calls, ...results].filter(Boolean).join('\n')
}
...
export function flattenTranscript(base, cfg) {
  const n = Math.max(0, Number(cfg.twinTranscriptMessages ?? 6))
  const limit = Number(cfg.twinBackgroundChars ?? 1500)
  if (!n) return ''
  const tail = base.slice(-n)
  const lines = []
  for (const msg of tail) {
    const body = readableText(msg)
    if (!body) continue
    const who = msg.role === 'assistant' ? '执行侧' : msg.role === 'system' ? '系统' : '用户/工具'
    lines.push(`【${who}】${body}`)
  }
  let out = lines.join('\n\n')
...
export function backgroundMessages(base, cfg) {
  ...
  // flatten：把最近若干条拍平成一段普通文本（并清掉特殊标识符），而不是交出可续写的对话结构。
  if (String(cfg.twinContextMode || '').toLowerCase() === 'flatten') {
    const transcript = flattenTranscript(base, cfg)
    if (!transcript) return []
    return [userMessage(`下面这段是执行智能体最近的记录，已经处理成普通文本（不是对话，不用接着写）：\n\n${transcript}`, 'instructions')]
  }
  for (let i = base.length - 1; i >= 0; i--) {
    ...
    const clean = cfg.twinStripMarkers === false ? body : stripSpecialTokens(body)
