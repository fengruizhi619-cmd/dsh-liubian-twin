...
export const DEFAULTS = {
  ...
  twinBackgroundChars: 0, // 全局字符上限（0 = 不限，由「轮数 × 每轮字数」决定预算）
  twinTranscriptRounds: 10, // flatten 模式下拍平最近多少**轮**（一轮 = 人类发言 + 其后动作/工具往返）
  twinRoundChars: 0, // 每轮字数上限（0 = 不限；用户指定：正文回答与用户输入不设上限）
  twinIncludeTools: false, // 拍平时是否把工具调用/工具结果也写进去（默认丢掉，只留用户输入与模型输出）
  twinIncludeReasoning: true, // 是否把模型的思考也拍进去（判"想干什么"很有用；嫌长可关）
 ...
 * `twinBackgroundChars` 是**全局上限**，0 = 不限（预算由「轮数 × 每轮字数」决定）。
 */
