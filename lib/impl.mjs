...
export const DEFAULTS = {
  ...
  // 监察转录字数上限（保尾截断，最近的最相关）。2026-09-26 实测：无上限时大会话
  // （604 回合）的监察指令膨胀到 39 万字 ≈ 24.5 万 token，单次调用占线 50-90 秒，
  // 多会话并发直接触发 provider 429/空补全——这是"监察成功率低"的主根因。
  twinTranscriptMaxChars: 24000,
 ...
 * **不设字数上限**（用户指定）。
 */
/** 监察转录字数上限：保尾截断（最近的内容最相关），超限给一行说明。 */
function capTranscript(flat, cfg) {
  const max = Number(cfg.twinTranscriptMaxChars) || 0
  const body = String(flat || '')
  if (!(max > 0) || body.length <= max) return body
  return `…（更早内容已按 twinTranscriptMaxChars=${max} 截断，只保留最近部分）\n` + body.slice(-max)
}
...
export function contextTextFor(base, cfg) {
  ...
  if (mode === 'flatten') return capTranscript(flattenTranscript(base, cfg), cfg)
...
export async function callTwin(ctx, cfg, agent, { kind, targetDesc, meta = {}, signal, log }) {
  ...
  // 全局串行：同一宿主里绝不并发打第二个监察请求。
  // 2026-09-26 实测（8 路并发压测）：provider（火山 Ark coding 档）有严格并发/限流
  // 额度，多会话监察 + 执行器同时压 → HTTP 429 / 空补全连锁失败。在途时新审查直接
  // skip（闸门放行该动作）——不排队，排队会把执行侧的工具窗口吊住几分钟。
  if (kind !== 'debug' && monitorInFlight > 0) {
    return { status: 'skipped', thinking: '', error: '已有监察调用在途（全局串行，避免 provider 并发限流）' }
  }
  ...
  monitorInFlight += 1
  try {
    return await runMonitorCall(ctx, cfg, { provider, model, messages, kind, signal, log })
  } finally {
    monitorInFlight -= 1
  }
}

/** 重试间隔：命中 429/限流时四倍退避（provider 并发额度被占时的正确姿势是等，不是挤）。 */
export function pickRetryDelayMs(lastError, cfg) {
  const base = Math.max(0, Number(cfg.twinRetryDelayMs) || 5000)
  return /429|rate[- ]?limit|限流|并发/i.test(String(lastError || '')) ? base * 4 : base
}

let monitorInFlight = 0

/** 一次监察调用的重试循环（callTwin 的互斥段）。 */
async function runMonitorCall(ctx, cfg, { provider, model, messages, kind, signal, log }) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    if (attempt < attempts) {
      const delay = pickRetryDelayMs(lastError, cfg)
      log?.warn?.(`[${PLUGIN_NAME}] 监察调用失败（第 ${attempt}/${attempts} 次）：${lastError}［${provider}/${model}］；${delay}ms 后重试`)
